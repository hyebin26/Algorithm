...
---
### 나의풀이
1. info, query를 split으로 구별하기 쉽게 나눈다.
2. queryScore은 항상 존재하므로, 현재의 queryScore보다 infoScore이 더 큰 경우를 구한다.
3. 경우 중에 각 항에 맞는 걸 비교하고 같을 시 cnt++을 해주고 끝나고 cnt를 출력한다.
- 효율성을 실패하였따. => 이진탐색을 해야함.
```jsx
function solution(info, query) {
    var answer = [];
    info = info.map(item => item.split(" "));
    query = query.map(item => item.split(" and ")).map((item2,index2)=>{
        const queryScore = Number(item2[3].split(" ")[1])
        let cnt = 0;
        for(let i = 0; i < info.length;i++){
            const infoScore = Number(info[i][4]);
            if(queryScore <= infoScore){
               if((item2[0] === "-" || item2[0] === info[i][0]) 
                  && (item2[1] === "-" || item2[1] === info[i][1]) 
                  && (item2[2] === "-" || item2[2] === info[i][2]) 
                  && (item2[3].split(" ")[0] === "-" || item2[3].split(" ")[0] === info[i][3])){
                   cnt++;
               }
            }
        }
